...
namespace WPFHexaEditor.Core.Bytes
{
    ...
    public sealed class ByteProvider : IDisposable, IByteProvider
    {
        ...
        public ByteProvider(byte[] byteName) => ByteName = byteName;
        ...
        public void SetBytes()
        {
            if (ByteName != null)
            {
                ...
                try
                {
                    _stream = new MemoryStream(ByteName, writable:true);// File.Open(FileName, FileMode.Open, FileAccess.ReadWrite, FileShare.Read); ;
                }
                catch
                {
                      _stream = new MemoryStream(ByteName, writable: false);
