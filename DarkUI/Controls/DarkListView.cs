...
namespace DarkUI.Controls
{
    public class DarkListView : DarkScrollView
    {
        ...
        [Category("Appearance")]
        [Description("Determines whether the size is shown with the list items.")]
        [DefaultValue(false)]
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            for (var i = top; i <= bottom; i++)
            {
                ...
                var rect2 = new Rectangle(0, i * ItemHeight, width, ItemHeight);
        ...
        private void UpdateItemSize(DarkListItem item, Graphics g)
        {
            ...
            if (ShowSize)
                size.Width += g.MeasureString(item.Size, Font).Width + 10;
        ...
        protected override void PaintContent(Graphics g)
        {
            ...
            for (var i = top; i <= bottom; i++)
            {
                var width = Math.Max(ContentSize.Width, Viewport.Width);
                var rect = new Rectangle(0, i * ItemHeight, width, ItemHeight);
                var rect2 = new Rectangle(width, i * ItemHeight, width, ItemHeight);
                // Background
                var odd = i % 2 != 0;
                ...
                using (var b = new SolidBrush(Items[i].TextColor))
                {
                    ...
                    var size = g.MeasureString(Items[i].Size, Font);
                    var txt = g.MeasureString(Items[i].Text, Font);
                    var modFont = new Font(Font, Items[i].FontStyle);

                    var modRect = new Rectangle(rect.Left + 2, rect.Top, width, rect.Height);
                    var SizemodRect = new Rectangle(rect.Right - (int)size.Width - 5 , rect.Top, rect.Width - (int)txt.Width, rect.Height);
                    if (ShowIcons)
                    ...
                    if (ShowSize && Items[i].Size != null)
                    {
                        g.DrawString(Items[i].Size, modFont, b, SizemodRect, stringFormat);
                        //g.DrawImageUnscaled(Items[i].Icon, new Point(rect.Left + 5, rect.Top + (rect.Height / 2) - (_iconSize / 2)));
                    }
